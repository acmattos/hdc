...
private const val DEBUG_MESSAGE = "[SCHEDULE_ENDPOINT] - Creating a schedule for a dentist..."
private const val INFO_MESSAGE = "[SCHEDULE_ENDPOINT] - Schedule created for a dentist successfully!"
    ...
    Feature("${ScheduleCommandController::class.java} existence") {
        Scenario("trace message generation") {
            ...
            And("""a successful ${ScheduleCommandController::class.java} instantiation""") {
                ...
                every { context.json(any<String>()) }.returns(context)
            }
            When("""#createScheduleForADentist is executed""") {
                controller.createScheduleForADentist(context)
            }
