...
private const val EXCEPTION_MESSAGE = "There is a schedule already defined for the given scheduleDentistId [12345678901234567890123456]!"
    ...
    Feature("${ScheduleCommandApplicationService::class.java} usage") {
        Scenario("handling ${CreateAScheduleForTheDentistCommand::class.java} successfully") {
            ...
            And("""a ${ScheduleCommandApplicationService::class.java} successfully instantiated""") {
                service = ScheduleCommandApplicationService(eventStore, repository)
            }
        ...
        Scenario("handling ${CreateAScheduleForTheDentistCommand::class.java} for a already registered dentist's schedule ") {
            ...
            And("""eventStore#findAllByField returns an event""") {
                every {
                    ...
                    CreateAScheduleForTheDentistEvent(
                        ScheduleRequestBuilder.buildCreateAScheduleForTheDentistRequest().toType() as CreateAScheduleForTheDentistCommand
                    )
                )
            }
            And("""eventStore#addEvent just runs""") {
            ...
            Then("""${AssertionFailedException::class.java} is raised with message""") {
                assertion.hasSameClassAs(AssertionFailedException(EXCEPTION_MESSAGE, SCHEDULE_ALREADY_DEFINED.code))
            }
